...
def revisar_datos_finales(df: pd.DataFrame, y: Optional[Any] = None) -> bool:
    ...
    if len(cat_cols) > 0:
        print(f"{C_RED}❌ ERROR FATAL: Hay {len(cat_cols)} columnas categóricas sin procesar.{C_RESET}")
        errores += 1
    else:
    ...
    if errores > 0:
        print(f"{C_BOLD}{C_RED}⚠️ RESULTADO: Auditoría falló con {errores} error(es) fatal(es).{C_RESET}")
        return False
    elif advertencias > 0:
