...
def imputar_nulos(
    ...
    if df_test is not None:
        ...
        missing_cols = [col for col in df_train_imp.columns if col not in df_test.columns]
        extra_cols = [col for col in df_test.columns if col not in df_train_imp.columns]
        if missing_cols or extra_cols:
            details = []
            if missing_cols:
                details.append(f"faltan en df_test: {missing_cols}")
            if extra_cols:
                details.append(f"sobran en df_test: {extra_cols}")
            raise DataValidationError(f"Las columnas de train y test no coinciden: {'; '.join(details)}")

        df_test_imp = df_test.copy().reindex(columns=df_train_imp.columns)
        if len(cols_num) > 0:
...
def escalar_caracteristicas(
    ...
    missing_cols = [col for col in columnas if col not in df_train_scaled.columns]
    if missing_cols:
        raise DataValidationError(f"No existe la columna indicada para escalar: {missing_cols}")
    ...
    if df_test is not None:
        ...
        missing_cols_test = [col for col in columnas if col not in df_test.columns]
        if missing_cols_test:
            raise DataValidationError(f"Las columnas indicadas para escalar faltan en df_test: {missing_cols_test}")

        df_test_scaled = df_test.copy().reindex(columns=df_train_scaled.columns)
